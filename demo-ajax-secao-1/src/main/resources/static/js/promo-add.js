...
$("#form-add-promo").submit( function (evt){
	...
	$.ajax({
		...
		error: function(xhr) {
			...
			$("#alert").addClass("alert alert-danger").text("Nao foi possivel salvar essa promocao.");
		},
		complete: function () {
	...
	if (url.length > 7) {
		$.ajax({
			...
			success: function( data ) {
				...
				$("#alert").addClass("alert alert-danger").text("Informacao recuperada.");
				
			},
			statusCode: {
				404: function() {
					$("#alert").addClass("alert alert-danger").text("Nenhuma informaao pode ser recuperada.");
				}
			},
			error: function() {
				$("#linkImagem").attr("src", "/images/promo-dark.png");
